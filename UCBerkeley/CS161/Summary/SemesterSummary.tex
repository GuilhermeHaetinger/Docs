...
\label{sec:orgd663285}
Before you read anything from this section, take a look at the Appendix section on \hyperref[orgf8c9fec]{Assembly code}! There is a lot of review on it needed for this part of the content. Now that that's out of the way, let's smash the stack.
\subsubsection{Format String Vulnerability}
\label{sec:orga2622d4}
  For this exploit, it \textbf{very} important to understand the layout of variables inside the stack. For this, see the \hyperref[org10d266d]{appendix notes} on it.
...
\subsection{Certificate Authorities - Revised}
\label{sec:org5b493b3}
Certificates should be trusted but verified. They should also be public as a transparency mechanism so that you can see the certificates issued in your name, avoiding fake ones. They can be expensive to maintain, that's why \emph{LetsEncrypt} exists. They issue short-term certificates, which lead to low costs. 
\subsection{Denial of Service Attacks}
\label{sec:org1ffb52b}
We have three main DoS attacks in the content. One we have already seen, which is SYN Flooding, and the others are:
\begin{itemize}
\item Resource Consumption \(\to\) Fix with quota;
\item Reflected Amplified Attack \(\to\) when the attacker makes a number of requests with the victim's IP, making it receive numerous answers and crowding the link layer.
\end{itemize}
...
\subsection{Firewall}
\label{sec:org23bcdd3}
I'm no getting into as much detail as Weaver on this case because I don't think there is much to it. The idea of a firewall is to mediate the connection between the internal network and the internet. It can restrict connections from happening or restrict only one side from sending messages. They are central controls that are easy to deploy but reduce the network's usefulness and trust malicious insiders. A single breach in the mediation allows an attacker to get inside the system and pivot itself to where it wants to be. the \texttt{allow/drop} command to specify the sending and receiving IP and port allowed for a connection. You can see more of this in the \hyperref[org5d0c7d0]{important commands} appendix section.
\subsection{TOR (The Onion Router)}
\label{sec:org12608c2}
Referred as the \emph{Dark web}, TOR is the protocol of anonymous communication. It's called the onion router because it has different layers that are peeled off as the message goes from router to router. The abstract way of describing the process is that there are entering and exit nodes, both of which are known by everyone, but in between them, there are many anonymous routers that pass your message around without knowing its destination or your identity (the exit node knows the destination). This is achievable by different layers of encryption. The client has a list of all the Onion routers and their public keys. \uline{Let's say your message is routed 3 times before getting to server. You agree on a different key for each router and encapsulate your message in the as such: E(K1, E(K2, E(K3, Enc\textsubscript{Message}))). The first node will be able to understand the following node's address and the message encrypted by K1. The second one will get the third address and the message encrypted by K2. The third one will get the final encrypted message with the address to the server you want to communicate with}.
...
\label{sec:org4ee3144}
\begin{itemize}
\item KSK \(\to\) Key signing key;
\item ZSK \(\to\) Zone signing key
\end{itemize}
\subsubsection{Process}
\label{sec:org650ac83}
...
\begin{itemize}
\item Hash the node's KSK and root's name with root's ZSK;
\item Check if it matches with the DS entry;
\item If it does, then this means the entry is trusted by root;
\item Knowing that the entry is trusted, we check if the DNSKey entry is properly signed with the KSK and get the ZSK;
\item The ZSK is how the RRSIG is generated, so we check that;
\item Repeat the process for its child node using the found ZSK instead of root's.
\end{itemize}
...
\subsubsection{Name existence}
\label{sec:org524df5a}
...
\begin{itemize}
\item NOERROR \(\to\) Name exists but not for the lookup type;
\item NXDOMAIN \(\to\) Name doesn't exist;
\end{itemize}
...
\subsubsection{Alternative ways of detection}
\label{sec:orgb704710}
\begin{itemize}
\item Signature based \(\to\) Looking for activities with known attack structure;
\item Vulnerability Signature based \(\to\) Look for activities that match known problems;
\item Specification based \(\to\) Specify what kind of activity is allowed;
\item Behavioral \(\to\) Look for evidence of compromise instead of attacks (good for novel attack detection);
\item \textbf{Honeypots} \(\to\) deploy system with no operational purpose, if it starts operating then there is something wrong;
\end{itemize}
\subsection{Dragonfly}
\label{sec:org81e6943}
...
\end{itemize}
\item \hyperref[fig:org5ed3963]{Pointer registers}
\begin{itemize}
...
\caption{\label{fig:org5ed3963}The Stack registers layout}
\end{figure}

\subsubsection{How do function calls work?}
\label{sec:org9cc9e19}
...
\subsection{Using GDB \& Shell Exploits}
\label{sec:org43ccef8}
GDB Commands:
...
In project 1 we used the command \texttt{invoke}, which runs a script in a deterministic environment: environment variables and arguments all in the same place every time. some simple invoke commands:

\begin{minted}[,bgcolor=yellow]{bash}
...
\end{minted}
\begin{minted}[,bgcolor=yellow]{javascript}
